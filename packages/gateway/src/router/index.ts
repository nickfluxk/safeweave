...
export class Router {
  ...
  async scanAll(request: ScanRequest, blockedScanners?: Set<string>, requestedScanners?: Set<string>): Promise<ScanResult> {
    const start = Date.now();
    const promises = Array.from(this.clients.entries())
      .filter(([name]) => !blockedScanners?.has(name))
  ...
  async healthCheck(): Promise<Record<string, { status: string; latency_ms: number }>> {
